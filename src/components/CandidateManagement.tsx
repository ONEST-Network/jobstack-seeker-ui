...
const CandidateManagement = () => {
  const { t } = useTranslation("candidatemanagement");
              ...
              <Tabs value={activeView} onValueChange={setActiveView} className="w-auto">
                <TabsList className="grid w-full grid-cols-3">
                  <TabsTrigger value="list" className="flex items-center gap-2">
                    ...
                    {t('candidateManagement.views.cards')}
                  </TabsTrigger>
                  <TabsTrigger value="table" className="flex items-center gap-2">
                    <Table className="h-4 w-4" />
                    {t('candidateManagement.views.table')}
                  </TabsTrigger>
                  <TabsTrigger
                    value="map"
                  ...
                  >
                    ...
                    {t('candidateManagement.views.map')}
                  </TabsTrigger>
                </TabsList>
              </Tabs>
      ...
      <div className="flex">
        ...
        {showFilters && (
          <div className="w-80 border-r border-border bg-background p-6">
            <h3 className="font-semibold mb-4">{t('candidateManagement.filters.title')}</h3>
            <div className="text-sm text-muted-foreground">
              {t('candidateManagement.filters.placeholder')}
            </div>
          </div>
        )}
