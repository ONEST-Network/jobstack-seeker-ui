          ...
          {isMobile ? (
            <div className="space-y-2">
              {user.managedCandidates.map((candidate) => (
                <Card
                  key={candidate.id}
                  className={`transition-all duration-200 cursor-pointer hover:bg-muted/50 ${
                    ...
                    <div className="p-2 space-y-1">
                      {user.managedCandidates.map((candidate) => (
                        <DropdownMenuItem
                          key={candidate.id}
                          onClick={() => handleProfileSelect(candidate.id)}
                          className={`flex items-center justify-between p-2 rounded-lg transition-colors cursor-pointer hover:bg-muted ${
                            tempSelectedCandidate?.id === candidate.id
